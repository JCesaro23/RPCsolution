...
/* Verdict Boca: ACCEPTED.                                         */
/* Run Time Boca (seconds): 11.918                                 */
/*                                                                 */
...
public class howmany2{
	...
	private static void pre_calculoDP(BigInteger[] primero, BigInteger[] segundo,
		...
		for(int i=1; i<=5000;i++) {
			...
			count1[i]= (int) Math.floor(1+(i*Math.log10(2)));
			count2[i]= (int) Math.floor(1+(i*Math.log10(5)));
			int max = Math.max(count1[i], count2[i]);
			String s1[]=String.valueOf(primero[i]).split("");
			String s2[]=String.valueOf(segundo[i]).split("");
			for(int k=0; k<max; k++) {
				if(k<count1[i])
					++digits[i][Integer.parseInt(s1[k])];
				if(k<count2[i])
					++digits2[i][Integer.parseInt(s2[k])];
			}
